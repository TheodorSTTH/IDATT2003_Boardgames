 ...
 * <p>
 * The board is serialized into a JSON format to be saved and loaded for game persistence.
 */
public class BoardSnakesAndLadders {
    @JsonProperty
    private HashMap<Integer, Event> events = new HashMap<Integer, Event>(); // Map of events associated with tile numbers
    @JsonProperty
    private int endTile = 90; // The tile number that marks the end of the game
    @JsonProperty
    private HashMap<String, Integer> playerPositions = new HashMap<String, Integer>(); // Player names mapped to their positions
    ...
    /**
     * Moves a player a given number of steps forward.
     * If the player lands on a tile with an event, the event’s action is applied,
     * and player position is updated accordingly.
     *
     ...
     * Because the player names are stored in the playerPositions map,
     * and the dobele serialization of the player names lead to their
     * Position being serialized set to zero when loding.
     * </p>
     *
     ...
     * 
     * <p>
     * This methode is based on a simuler method suggested by ChatGPT.
     * <p>
     *
    ...
    public static BoardSnakesAndLadders loadBoard(String gameName) {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            return objectMapper.readValue(new File("src/main/resources/files/" + gameName + ".board.json"), BoardSnakesAndLadders.class);
        } catch (IOException e) {
