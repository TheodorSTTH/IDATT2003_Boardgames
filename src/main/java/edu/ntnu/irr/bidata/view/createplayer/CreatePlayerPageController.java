...
public class CreatePlayerPageController {
  ...
  private void initialize() {
    view.getCreatePlayerButton().setOnAction(e -> {
    ...
    view.getPlayerColorField().valueProperty().addListener((obs, oldFrom, newFrom) -> {
      String username = view.getUsernameField().getText();
      Button createPlayerButton = view.getCreatePlayerButton();
      createPlayerButton.setDisable(newFrom == null || username.isEmpty());
    });

    view.getUsernameField().textProperty().addListener((obs, oldFrom, newFrom) -> {
      Button createPlayerButton = view.getCreatePlayerButton();
      String playerColor = view.getPlayerColorField().getValue();
      createPlayerButton.setDisable(newFrom == null || newFrom.isEmpty() || playerColor == null);
    });
  }
